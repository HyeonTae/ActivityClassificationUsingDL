...
scalers = [None, "orig", "minmax", "maxabs", "robust", "QT-norm", "QT-uni", "standard", "PT-yj", "normalizer"]
weights = [None, "0.8", "0.8", "0.9", "0.6", "0.4", "0.6", "0.6", "0.6", "0.8"]
ground_truth_path = "../ground_truth/"

total_best_result = dict()

def Purity(data_name, ground_truth, clustering_result):
    g_result = dict()
...
def NMI(data_name, ground_truth_labels, ground_truth, clustering_result):
...
def ARI(data_name, ground_truth_labels, ground_truth, clustering_result):
    clustering_resul_labels = copy.deepcopy(ground_truth_labels)
    for g_key in ground_truth.keys():
        for g in ground_truth[g_key]:
            ground_truth_labels[ground_truth_labels.index(g)] = g_key
...
for data_name in ["rico", "seq2seq"]:
    ...
    for i, clustering_algorithm in enumerate(clustering_algorithms):
        ...
        result_path = 'result/' + data_name + "/" + clustering_algorithm + "/"
        ...
        if args.evaluation == "purity":
            result_dict[clustering_algorithm].append(
                    Purity(data_name, ground_truth, clustering_result))
        elif args.evaluation == "nmi":
            result_dict[clustering_algorithm].append(
                    NMI(data_name, ground_truth_labels, ground_truth, clustering_result))
        elif args.evaluation == "ari":
            result_dict[clustering_algorithm].append(
                    ARI(data_name, ground_truth_labels, ground_truth, clustering_result))

    save_csv_file(result_dict)

for f in fusion_types:
    for s, w in zip(scalers, weights):
        fusion_type = "_" + f if f is not None else ""
        scaler = "_" + s if s is not None else ""
        weight = "_" + w if w is not None else ""
